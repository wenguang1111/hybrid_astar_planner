...
vector<DubinsPath> HybridAStar::getNeighbors(Pose &p) {
    ...
    DubinsPath straight(1,
                        make_pair(direction_t::straight, hastar_hp->step_size));
...
vector<Pose> HybridAStar::runHybridAStar() {
    ...
    while (!openlist.empty() && count < hastar_hp->max_iterations) {
        ...
        vector<Pose> path =
            Dubins::generatePath(x.pose, shortest_dp, hastar_hp->radius);
        if (!isCollision(path) &&
        ...
        if (distance(x.pose, map_info->end) <=
                hastar_hp->completion_threshold &&
            abs(x.pose[2] - map_info->end[2]) <=
                hastar_hp->angle_completion_threshold) {
            best_pose = x.pose;
            break;
        }
        ...
        for (auto neighbor : neighbors) {
            ...
            if (std::find_if(closelist.begin(), closelist.end(),
                             [&](HybridAStarPoint &p) {
                                 return (distance(p.pose, y) <
                                         hastar_hp->completion_threshold);
                             }) != closelist.end()) {
                continue;
            }
